...
class SupplierPriceListController extends Controller
{
    ...
    private array $supplierPresets = [
        'fortinet' => [
            ...
            'columnPatterns' => [
                ...
                'price_1yr' => ['1yr Contract', '1 Year', '1Yr', 'Replaces DD by 12'],
                'price_2yr' => ['2yr Contract', '2 Year', '2Yr', 'Replaces DD by 24'],
                'price_3yr' => ['3yr Contract', '3 Year', '3Yr', 'Replaces DD by 36'],
                'price_4yr' => ['4yr Contract', '4 Year', '4Yr', 'Replaces DD by 48'],
                'price_5yr' => ['5yr Contract', '5 Year', '5Yr', 'Replaces DD by 60'],
            ],
        ],
        'cisco' => [
            'name' => 'Cisco',
        ...
        try {
            ...
            foreach ($request->sheets as $sheetConfig) {
                ...
                for ($row = $headerRow + 1; $row <= $highestRow; $row++) {
                    ...
                    $hasPrice = $itemData['list_price'] || $itemData['price_1yr'] || $itemData['price_3yr'] || $itemData['price_5yr'];
                    if (!$hasPrice) {
                        $sheetLog['items_skipped']++;
                        continue;
                    }
    ...
    {
        $headerKeywords = ['sku', 'part number', 'product', 'description', 'price', 'item', 'unit'];
        
        // Thử dòng được suggest trước
        $rowsToCheck = [$suggestedRow];
        
        // Thêm các dòng phổ biến cho Fortinet (4-8)
        for ($i = 1; $i <= 15; $i++) {
        ...
        foreach ($preset['columnPatterns'] as $field => $patterns) {
            foreach ($patterns as $pattern) {
                ...
                foreach ($fileHeaders as $index => $headerName) {
                    ...
                    $isMatch = $headerName === $patternLower || 
                               str_contains($headerName, $patternLower);
                    
                    if ($isMatch) {
                        $mapping[$field] = $index;
                        Log::debug("Mapped field '{$field}' to column {$index} (header: {$headerName}, pattern: {$patternLower})");
                        break 2;
                    }
                }
            }
