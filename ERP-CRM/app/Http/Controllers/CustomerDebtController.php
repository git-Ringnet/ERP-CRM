    ...
    public function agingReport(Request $request)
    {
        ...
        foreach ($sales as $sale) {
            ...
            if ($daysPast <= 30) {
                ...
                $aging['days_31_60'] += $sale->debt_amount;
            } elseif ($daysPast <= 90) {
                $aging['days_61_90'] += $sale->debt_amount;
            } else {
                $aging['over_90'] += $sale->debt_amount;
            }
        }

        $customers = Customer::orderBy('name')->get(['id', 'name', 'code']);

        return view('customer-debts.aging-report', compact('aging', 'sales', 'customers', 'customerId'));
    }
}
