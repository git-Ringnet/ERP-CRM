        ...
        <form action="{{ route('cost-formulas.update', $costFormula->id) }}" method="POST">
            ...
            <div class="p-4 sm:p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" required
                            ...
                            <option value="shipping" {{ old('type', $costFormula->type) == 'shipping' ? 'selected' : '' }}>Vận
                                chuyển</option>
                            <option value="marketing" {{ old('type', $costFormula->type) == 'marketing' ? 'selected' : '' }}>
                                Marketing</option>
                            <option value="commission" {{ old('type', $costFormula->type) == 'commission' ? 'selected' : '' }}>Hoa hồng</option>
                            <option value="other" {{ old('type', $costFormula->type) == 'other' ? 'selected' : '' }}>Khác
                            </option>
                        </select>
                    </div>
                ...
                <div id="fixedField" class="hidden">
                    ...
                    <input type="number" name="fixed_amount" value="{{ old('fixed_amount', $costFormula->fixed_amount) }}"
                        min="0"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                ...
                <div id="percentageField" class="hidden">
                    ...
                    <input type="number" name="percentage" value="{{ old('percentage', $costFormula->percentage) }}" min="0"
                        max="100" step="0.01"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
                ...
                <div>
                    ...
                    <select name="apply_to" id="applyTo" required onchange="toggleApplyFields()"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
                        <option value="all" {{ old('apply_to', $costFormula->apply_to) == 'all' ? 'selected' : '' }}>Tất cả
                            đơn hàng</option>
                        <option value="product" {{ old('apply_to', $costFormula->apply_to) == 'product' ? 'selected' : '' }}>
