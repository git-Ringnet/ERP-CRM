...
@section('title', 'Chỉnh sửa giá nhập')
@section('page-title', 'Chỉnh sửa giá nhập')
...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('purchase-pricings.show', $purchasePricing) }}" class="inline-flex items-center px-3 py-1.5 text-sm bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200">
            <i class="fas fa-eye mr-2"></i>Xem chi tiết
        </a>
    </div>

    <form action="{{ route('purchase-pricings.update', $purchasePricing) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div class="p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="product_id" required class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                                    <option value="">Chọn sản phẩm</option>
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" {{ old('product_id', $purchasePricing->product_id) == $product->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="supplier_id" required class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                                    <option value="">Chọn NCC</option>
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ old('supplier_id', $purchasePricing->supplier_id) == $supplier->id ? 'selected' : '' }}>
                                            {{ $supplier->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="purchase_order_id" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                                    ...
                                    @foreach($purchaseOrders as $po)
                                        <option value="{{ $po->id }}" {{ old('purchase_order_id', $purchasePricing->purchase_order_id) == $po->id ? 'selected' : '' }}>
                                            {{ $po->code }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="quantity" value="{{ old('quantity', $purchasePricing->quantity) }}" min="1" required
                    ...
                    <div class="p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="purchase_price" id="purchase_price" value="{{ old('purchase_price', $purchasePricing->purchase_price) }}" min="0" required onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Chiết khấu NCC (%)</label>
                                <input type="number" name="discount_percent" id="discount_percent" value="{{ old('discount_percent', $purchasePricing->discount_percent) }}" min="0" max="100" step="0.1" onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="number" name="vat_percent" id="vat_percent" value="{{ old('vat_percent', $purchasePricing->vat_percent) }}" min="0" max="100" onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="shipping_cost" id="shipping_cost" value="{{ old('shipping_cost', $purchasePricing->shipping_cost) }}" min="0" onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Chi phí bốc xếp (VNĐ)</label>
                                <input type="number" name="loading_cost" id="loading_cost" value="{{ old('loading_cost', $purchasePricing->loading_cost) }}" min="0" onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Chi phí kiểm tra (VNĐ)</label>
                                <input type="number" name="inspection_cost" id="inspection_cost" value="{{ old('inspection_cost', $purchasePricing->inspection_cost) }}" min="0" onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Chi phí khác (VNĐ)</label>
                                <input type="number" name="other_cost" id="other_cost" value="{{ old('other_cost', $purchasePricing->other_cost) }}" min="0" onchange="calculatePrices()"
                                       class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                            </div>
                            <div>
            ...
            <div class="space-y-4">
                    ...
                    <div class="p-4">
                        <div class="mb-4">
                            ...
                            <select name="pricing_method" required class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
                                <option value="average" {{ old('pricing_method', $purchasePricing->pricing_method) == 'average' ? 'selected' : '' }}>Bình quân gia quyền</option>
                                <option value="fifo" {{ old('pricing_method', $purchasePricing->pricing_method) == 'fifo' ? 'selected' : '' }}>FIFO</option>
                                <option value="lifo" {{ old('pricing_method', $purchasePricing->pricing_method) == 'lifo' ? 'selected' : '' }}>LIFO</option>
                            </select>
                        </div>
                        <div>
                ...
                <div class="bg-white rounded-lg shadow-sm">
                    ...
                    <div class="p-4">
                        <textarea name="note" rows="4" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">{{ old('note', $purchasePricing->note) }}</textarea>
                    </div>
                </div>
