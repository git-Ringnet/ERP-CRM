    ...
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <div class="p-3 sm:p-4 border-b border-gray-200 space-y-3">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <!-- Filter by Type -->
                <select name="type"
                    onchange="window.location.href='{{ route('damaged-goods.index') }}?type='+this.value+'&status={{ request('status') }}&search={{ request('search') }}'"
                ...
                <!-- Filter by Status -->
                <select name="status"
                    onchange="window.location.href='{{ route('damaged-goods.index') }}?status='+this.value+'&type={{ request('type') }}&search={{ request('search') }}'"
                    class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('damaged-goods.export', request()->query()) }}"
                        class="inline-flex items-center justify-center px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm">
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sản phẩm
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Số lượng
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Giá trị
                            gốc</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Thu hồi
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tổn thất
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ngày phát
                            hiện</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($damagedGoods as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span
                                    class="px-2 py-1 text-xs font-semibold rounded-full {{ $item->type === 'damaged' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    {{ $item->getTypeLabel() }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 text-sm text-red-600 font-medium">
                                {{ number_format($item->getLossAmount(), 0) }}đ
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-600">{{ $item->discovery_date->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                @if($item->status == 'pending')
                                    <span
                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">{{ $item->getStatusLabel() }}</span>
                                @elseif($item->status == 'approved')
                                    <span
                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">{{ $item->getStatusLabel() }}</span>
                                @elseif($item->status == 'rejected')
                                    <span
                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">{{ $item->getStatusLabel() }}</span>
                                @else
                                    <span
                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{{ $item->getStatusLabel() }}</span>
                                @endif
                            </td>
            ...
            @forelse($damagedGoods as $item)
                <div class="p-4 hover:bg-gray-50">
                    ...
                    <div class="space-y-1 text-sm text-gray-600 mb-3">
                        ...
                        <div><i class="fas fa-money-bill w-4"></i> Giá trị gốc: {{ number_format($item->original_value, 0) }}đ
                        </div>
                        <div><i class="fas fa-undo w-4"></i> Thu hồi: {{ number_format($item->recovery_value, 0) }}đ</div>
                        <div class="text-red-600"><i class="fas fa-chart-line w-4"></i> Tổn thất:
                            {{ number_format($item->getLossAmount(), 0) }}đ
                        </div>
