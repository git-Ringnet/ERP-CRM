        ...
        <form action="{{ route('roles.store') }}" method="POST">
            ...
            <div class="mb-4">
                <label for="slug" class="block text-sm font-medium text-gray-700 mb-2">
                    Mã Vai trò (Slug) <span class="text-gray-400 text-xs">(Nếu trống sẽ tự động tạo từ tên)</span>
                </label>
                <input type="text" 
                       ...
                       id="slug" name="slug" value="{{ old('slug') }}" placeholder="ví dụ: admin_ketoan">
                @error('slug')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500 italic">Chỉ dùng chữ thường, số, gạch ngang và gạch dưới.</p>
            </div>
