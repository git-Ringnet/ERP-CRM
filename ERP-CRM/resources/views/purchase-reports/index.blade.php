...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <button onclick="window.location.reload()" class="inline-flex items-center px-3 py-1.5 text-sm bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">
                <i class="fas fa-sync mr-2"></i>Làm mới
            </button>
            <a href="{{ route('purchase-reports.export') }}" class="inline-flex items-center px-3 py-1.5 text-sm bg-green-500 text-white rounded-md hover:bg-green-600">
                <i class="fas fa-file-export mr-2"></i>Xuất Excel
            </a>
            <button onclick="window.print()" class="inline-flex items-center px-3 py-1.5 text-sm bg-blue-500 text-white rounded-md hover:bg-blue-600">
    ...
    <div class="bg-white rounded-lg shadow-sm p-4">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
                ...
                <input type="date" name="date_from" value="{{ $dateFrom }}" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Đến ngày</label>
                <input type="date" name="date_to" value="{{ $dateTo }}" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nhà cung cấp</label>
                <select name="supplier_id" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
            ...
            <div>
                ...
                <select name="product_id" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary">
    ...
    <div class="bg-white rounded-lg shadow-sm">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px" id="reportTabs">
                <button type="button" class="tab-btn active px-4 py-3 text-sm font-medium border-b-2 border-primary text-primary" data-tab="supplier">
                    <i class="fas fa-building mr-1"></i>Theo NCC
                </button>
                <button type="button" class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700" data-tab="product">
                    <i class="fas fa-box mr-1"></i>Theo sản phẩm
                </button>
                <button type="button" class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700" data-tab="monthly">
                    <i class="fas fa-calendar-alt mr-1"></i>Theo tháng
                </button>
                <button type="button" class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700" data-tab="cost">
                    <i class="fas fa-chart-pie mr-1"></i>Phân tích CP
                </button>
                <button type="button" class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700" data-tab="discount">
        ...
        <div class="tab-content p-4" id="tab-supplier">
            <h3 class="text-base font-semibold text-gray-800 mb-4"><i class="fas fa-building mr-2 text-primary"></i>Báo cáo theo nhà cung cấp</h3>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($supplierReport as $row)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2 text-right">{{ number_format($row['total_subtotal'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right text-green-600">{{ number_format($row['total_discount'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['total_shipping'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right font-bold">{{ number_format($row['total_paid'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-center"><span class="inline-block px-2 py-0.5 text-xs font-medium bg-green-100 text-green-800 rounded-full">{{ $row['discount_rate'] }}%</span></td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-3 py-8 text-center text-gray-500">Không có dữ liệu</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="tab-content p-4 hidden" id="tab-product">
            <h3 class="text-base font-semibold text-gray-800 mb-4"><i class="fas fa-box mr-2 text-primary"></i>Báo cáo theo sản phẩm</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($productReport as $row)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2 text-right">{{ number_format($row['avg_purchase_price'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['total_value'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right text-green-600 font-bold">{{ number_format($row['avg_warehouse_price'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['total_service_cost'], 0, ',', '.') }}đ</td>
        ...
        <div class="tab-content p-4 hidden" id="tab-monthly">
            <h3 class="text-base font-semibold text-gray-800 mb-4"><i class="fas fa-calendar-alt mr-2 text-primary"></i>Báo cáo theo tháng</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50">
        ...
        <div class="tab-content p-4 hidden" id="tab-cost">
            <h3 class="text-base font-semibold text-gray-800 mb-4"><i class="fas fa-chart-pie mr-2 text-primary"></i>Phân tích chi phí</h3>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">Giá trị hàng hóa</p>
                    <p class="text-xl font-bold text-primary">{{ number_format($costAnalysis['goods_value'] / 1000000, 1) }}tr</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">Chi phí vận chuyển</p>
                    <p class="text-xl font-bold text-yellow-600">{{ number_format($costAnalysis['shipping_cost'] / 1000000, 1) }}tr</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">Chi phí phục vụ</p>
                    <p class="text-xl font-bold text-blue-600">{{ number_format($costAnalysis['service_cost'] / 1000000, 1) }}tr</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">VAT</p>
                    <p class="text-xl font-bold text-red-600">{{ number_format($costAnalysis['vat_amount'] / 1000000, 1) }}tr</p>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
        ...
        <div class="tab-content p-4 hidden" id="tab-discount">
            <h3 class="text-base font-semibold text-gray-800 mb-4"><i class="fas fa-tags mr-2 text-primary"></i>Phân tích chiết khấu</h3>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">CK cơ bản</p>
                    <p class="text-xl font-bold text-green-600">{{ number_format($discountAnalysis['totals']['base'] / 1000000, 1) }}tr</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">CK số lượng</p>
                    <p class="text-xl font-bold text-blue-600">{{ number_format($discountAnalysis['totals']['volume'] / 1000000, 1) }}tr</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">CK thanh toán sớm</p>
                    <p class="text-xl font-bold text-yellow-600">{{ number_format($discountAnalysis['totals']['early'] / 1000000, 1) }}tr</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 text-center">
                    <p class="text-sm text-gray-600">CK đặc biệt</p>
                    <p class="text-xl font-bold text-red-600">{{ number_format($discountAnalysis['totals']['special'] / 1000000, 1) }}tr</p>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($discountAnalysis['by_supplier'] as $row)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2 font-medium">{{ $row['supplier'] }}</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['base_discount'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['volume_discount'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['early_discount'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right">{{ number_format($row['special_discount'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-right text-green-600 font-bold">{{ number_format($row['total_discount'], 0, ',', '.') }}đ</td>
                                <td class="px-3 py-2 text-center"><span class="inline-block px-2 py-0.5 text-xs font-medium bg-green-100 text-green-800 rounded-full">{{ $row['discount_rate'] }}%</span></td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-3 py-8 text-center text-gray-500">Không có dữ liệu</td></tr>
