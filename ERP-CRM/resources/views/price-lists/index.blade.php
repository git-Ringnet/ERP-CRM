    ...
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex flex-col sm:flex-row gap-4 flex-1">
                ...
                <div class="relative flex-1 max-w-md">
                    <form action="{{ route('price-lists.index') }}" method="GET" class="flex">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm kiếm bảng giá..."
                ...
                <div class="flex items-center gap-2">
                    <select name="type"
                        onchange="window.location.href='{{ route('price-lists.index') }}?type='+this.value+'&status={{ request('status') }}&search={{ request('search') }}'"
                        ...
                        <option value="customer" {{ request('type') == 'customer' ? 'selected' : '' }}>Theo khách hàng
                        </option>
                ...
                <div class="flex items-center gap-2">
                    <select name="status"
                        onchange="window.location.href='{{ route('price-lists.index') }}?status='+this.value+'&type={{ request('type') }}&search={{ request('search') }}'"
                        class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div class="flex gap-2">
                <a href="{{ route('price-lists.export', request()->query()) }}"
        ...
        <div class="mx-4 mt-4 bg-blue-50 border-l-4 border-blue-500 p-3 rounded">
            <p class="text-blue-700 text-sm"><i class="fas fa-info-circle mr-2"></i>Tạo nhiều bảng giá theo khách hàng,
                khuyến mãi, giá sỉ. Hệ thống tự động áp dụng bảng giá phù hợp.</p>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tên bảng
                            giá</th>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Khách
                            hàng</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Thời gian
                        </th>
                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Số SP
                        </th>
            ...
            @forelse($priceLists as $priceList)
                <div class="p-4 hover:bg-gray-50">
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1">
                            <a href="{{ route('price-lists.show', $priceList) }}"
                                class="font-medium text-blue-600 hover:text-blue-800">{{ $priceList->code }}</a>
                    ...
                    <div class="space-y-1 text-sm text-gray-600 mb-3">
                        ...
                        <div><i class="fas fa-user w-4"></i>
                            {{ $priceList->customer ? $priceList->customer->name : 'Tất cả khách hàng' }}</div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <form action="{{ route('price-lists.toggle', $priceList) }}" method="POST" class="flex-1">
                            ...
                            <button type="submit"
                                ...
                                <i
                                    class="fas {{ $priceList->is_active ? 'fa-toggle-off' : 'fa-toggle-on' }} mr-1"></i>{{ $priceList->is_active ? 'Tắt' : 'Bật' }}
                            </button>
                        </form>
                    </div>
                </div>
