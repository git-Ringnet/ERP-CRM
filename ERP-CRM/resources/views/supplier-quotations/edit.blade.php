                        ...
                        <select name="currency_id" id="currencySelect" onchange="onCurrencyChange()"
                            ...
                            @foreach($currencies as $currency)
                                <option value="{{ $currency->id }}"
                                    ...
                                    {{ old('currency_id', $supplierQuotation->currency_id ?? $baseCurrencyId) == $currency->id ? 'selected' : '' }}>
                                    {{ $currency->code }} - {{ $currency->name_vi }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div id="exchangeRateGroup" class="{{ ($supplierQuotation->currency_id && $supplierQuotation->currency_id != $baseCurrencyId) ? '' : 'hidden' }}">
                        ...
                        <input type="number" name="exchange_rate" id="exchangeRateInput" step="0.000001" min="0"
                            value="{{ old('exchange_rate', $supplierQuotation->exchange_rate ? floatval($supplierQuotation->exchange_rate) : 1) }}"
