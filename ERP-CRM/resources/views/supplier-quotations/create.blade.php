...
<div class="bg-white rounded-lg shadow-sm">
    ...
    <form action="{{ route('supplier-quotations.store') }}" method="POST" class="p-4">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <p class="text-blue-800"><i class="fas fa-info-circle mr-2"></i> Nhập báo giá cho yêu cầu: <strong>{{ $selectedRequest->code }}</strong> - {{ $selectedRequest->title }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div>
                ...
                <select name="supplier_id" id="supplierSelect" required class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg">
                    <option value="">Tìm và chọn nhà cung cấp...</option>
                    @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->id }}" data-email="{{ $supplier->email }}" data-phone="{{ $supplier->phone }}">{{ $supplier->code }} - {{ $supplier->name }}</option>
                    @endforeach
                </select>
            </div>
            @if(!$selectedRequest)
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Ngày báo giá <span class="text-red-500">*</span></label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <textarea name="note" rows="3" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg">{{ old('note') }}</textarea>
            </div>
            <div class="space-y-3">
