...
@section('title', 'Sao kê công nợ KH - ' . $customer->name)
@section('page-title', 'Sao kê công nợ khách hàng')
        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <div class="flex items-center text-sm text-gray-500 mb-2">
                    <a href="{{ route('customer-debts.index') }}" class="hover:text-blue-600">Công nợ KH</a>
                    <i class="fas fa-chevron-right mx-2 text-xs"></i>
                    <a href="{{ route('customer-debts.show', $customer) }}" class="hover:text-blue-600">{{ $customer->name }}</a>
                    <i class="fas fa-chevron-right mx-2 text-xs"></i>
                    <span class="text-gray-800">Sao kê</span>
                </div>
                <h2 class="text-xl font-bold text-gray-800">
                    <i class="fas fa-file-alt text-indigo-600 mr-2"></i>Sao kê công nợ - {{ $customer->name }}
                </h2>
            </div>
            <div class="flex gap-2">
                ...
                <a href="{{ route('customer-debts.export-statement', [$customer, 'date_from' => $dateFrom, 'date_to' => $dateTo]) }}"
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-6 border-b border-gray-200">
            <div class="text-center">
                <h2 class="text-2xl font-bold text-gray-800">SAO KÊ CÔNG NỢ KHÁCH HÀNG</h2>
            ...
            <div class="grid grid-cols-2 gap-4 mt-4 text-sm">
                <div>
                    <p><strong>KH:</strong> {{ $customer->name }}</p>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ngày ghi nợ</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm">
                            @if($txn['type'] === 'debit')
                                <span class="px-2 py-0.5 rounded text-xs bg-red-100 text-red-800">Bán hàng</span>
                            @else
        ...
        <div class="p-6 border-t border-gray-200">
            <div class="grid grid-cols-2 gap-8 text-center text-sm">
                <div>
                    <p class="font-semibold text-gray-800">Bên bán</p>
                    <p class="text-gray-500 mt-4">(Ký, ghi rõ họ tên)</p>
                </div>
                <div>
                    <p class="font-semibold text-gray-800">Bên mua (KH)</p>
                    <p class="text-gray-500 mt-4">(Ký, ghi rõ họ tên)</p>
                </div>
            </div>
        </div>
