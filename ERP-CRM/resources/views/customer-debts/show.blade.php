                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Trạng thái nợ</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($sales as $sale)
                            @php
                                ...
                                $debtDays = $customer->debt_days ?? 30;
                                $dueDate = $sale->date->copy()->addDays($debtDays);
                                $daysRemaining = now()->diffInDays($dueDate, false);
                                $isOverdue = $daysRemaining < 0 && $sale->debt_amount > 0;
                                $isDueSoon = $daysRemaining >= 0 && $daysRemaining <= 7 && $sale->debt_amount > 0;
                                $isPaid = $sale->debt_amount <= 0;

                                // Row highlight class
                                $rowClass = $isOverdue ? 'bg-red-50 hover:bg-red-100' : ($isDueSoon ? 'bg-orange-50 hover:bg-orange-100' : 'hover:bg-gray-50');
                            @endphp
                            <tr class="{{ $rowClass }}">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div
                                        class="{{ $isOverdue ? 'text-red-600' : ($isDueSoon ? 'text-orange-600' : 'text-gray-600') }}">
                                        {{ $dueDate->format('d/m/Y') }}
                                    </div>
                                    @if($sale->debt_amount > 0)
                                        @if($isOverdue)
                                            <div class="text-xs text-red-500 font-medium flex items-center mt-1">
                                                <i class="fas fa-exclamation-triangle mr-1"></i> Quá hạn {{ abs($daysRemaining) }} ngày
                                            </div>
                                        @elseif($isDueSoon)
                                            <div class="text-xs text-orange-500 font-medium flex items-center mt-1">
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if($isPaid)
                                        <span
                                            ...
                                            <i class="fas fa-check-circle mr-1"></i> Đã TT
                                        </span>
                                    @elseif($isOverdue)
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 animate-pulse">
                                            <i class="fas fa-exclamation-circle mr-1"></i> Quá hạn
                                        </span>
                                    @elseif($isDueSoon)
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                                            <i class="fas fa-clock mr-1"></i> Sắp đến hạn
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            <i class="fas fa-hourglass-half mr-1"></i> Trong hạn
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-center">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($paymentHistories as $payment)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <form action="{{ route('customer-debts.delete-payment', $payment) }}" method="POST"
                                        class="inline"
            ...
            <form id="paymentForm" method="POST">
                ...
                <div class="p-6 space-y-4">
                    <div class="bg-gray-50 p-3 rounded-lg">
                        <p class="text-sm text-gray-500">Đơn hàng: <span id="modalSaleCode"
                                class="font-medium text-gray-800"></span></p>
                        <p class="text-sm text-gray-500">Công nợ còn lại: <span id="modalDebtAmount"
                                class="font-medium text-red-600"></span></p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Số tiền thanh toán <span
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Phương thức <span
                                class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Ngày thanh toán <span
                                class="text-red-500">*</span></label>
