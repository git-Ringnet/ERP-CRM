...
<div class="max-w-5xl mx-auto space-y-6" x-data="{ 
    openCust: false, custSearch: '', customerId: '{{ old('customer_id') }}', customerName: '',
    openSales: false, salesSearch: '', salesOwnerId: '{{ old('sales_owner_id', Auth::id()) }}', salesOwnerName: '',
    openLead: false, leadSearch: '', teamLeadId: '{{ old('team_lead_id') }}', teamLeadName: '',
    openEng: false, engSearch: '', assignedTo: '{{ old('assigned_to') }}', assignedToName: '',
    ...
    <form method="POST" action="{{ route('technical-tickets.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" required placeholder="Ví dụ: Triển khai tường lửa Fortigate cho Khách hàng A" class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary @error('title') border-red-500 @enderror" value="{{ old('title') }}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="work_type" id="work_type" required class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary @error('work_type') border-red-500 @enderror">
                            ...
                            <option value="survey" {{ old('work_type') === 'survey' ? 'selected' : '' }}>Khảo sát / Tư vấn / Thiết kế</option>
                            <option value="BOM" {{ old('work_type') === 'BOM' ? 'selected' : '' }}>BOM Support</option>
                            <option value="documentation" {{ old('work_type') === 'documentation' ? 'selected' : '' }}>Technical Documents</option>
                            <option value="POC" {{ old('work_type') === 'POC' ? 'selected' : '' }}>POC / Demo</option>
                            <option value="deployment" {{ old('work_type') === 'deployment' ? 'selected' : '' }}>Deployment</option>
                            <option value="after_sales" {{ old('work_type') === 'after_sales' ? 'selected' : '' }}>After-sales support</option>
                            <option value="training" {{ old('work_type') === 'training' ? 'selected' : '' }}>Training / Update</option>
                            <option value="event" {{ old('work_type') === 'event' ? 'selected' : '' }}>Event / Speaker</option>
                    ...
                    <div>
                        ...
                        <input type="text" readonly class="w-full border-gray-200 rounded-lg text-sm bg-gray-50 text-gray-500 cursor-not-allowed" value="{{ Auth::user()->name }}">
                    </div>

                    <!-- Department -->
                    <div>
                        <label for="department" class="block text-sm font-semibold text-gray-700 mb-1">Bộ phận yêu cầu</label>
                        <input type="text" name="department" id="department" list="department_list" class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary" value="{{ old('department', Auth::user()->department ?? 'Sales') }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="project_name" id="project_name" placeholder="Nhập tên dự án, đối tác hoặc người dùng cuối..." class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary" value="{{ old('project_name') }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="sla_deadline" id="sla_deadline" class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary" value="{{ old('sla_deadline') }}">
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select name="opportunity_id" id="opportunity_id" class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary">
                                ...
                                @foreach($opportunities as $opp)
                                    <option value="{{ $opp->id }}" {{ old('opportunity_id') == $opp->id ? 'selected' : '' }}>{{ $opp->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="sale_id" id="sale_id" class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary">
                                <option value="">-- Không chọn / Không có --</option>
                                @foreach($sales as $s)
                                    <option value="{{ $s->id }}" {{ old('sale_id') == $s->id ? 'selected' : '' }}>{{ $s->code }} - {{ $s->customer_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary">
                            ...
                            <option value="waiting" {{ old('status') === 'waiting' ? 'selected' : '' }}>Waiting (Customer/Partner/Vendor)</option>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="6" placeholder="Ghi nhận thông tin cụ thể về nội dung hỗ trợ, yêu cầu kỹ thuật..." class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary">{{ old('description') }}</textarea>
                </div>

                <!-- Solution / Evaluation -->
                <div>
                    <label for="solution" class="block text-sm font-semibold text-gray-700 mb-1">Nguyên nhân / Phương án / Cách xử lý</label>
                    <textarea name="solution" id="solution" rows="4" placeholder="Kỹ sư hoặc Lead điền nguyên nhân, phương án xử lý, cấu hình chi tiết..." class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary">{{ old('solution') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                <div>
                    <label for="attachments" class="block text-sm font-semibold text-gray-700 mb-1">Chọn tài liệu phát sinh (BOM, Biên bản, Kế hoạch, Giải pháp...)</label>
                    <input type="file" name="attachments[]" id="attachments" multiple class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 border border-dashed border-gray-300 rounded-lg p-4 cursor-pointer focus:outline-none">
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 flex justify-end space-x-3 shadow-sm">
            <a href="{{ route('technical-tickets.index') }}" class="px-4 py-2 border border-gray-300 bg-white rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
                Hủy bỏ
            </a>
            <button type="submit" class="px-5 py-2 bg-primary hover:bg-primary/95 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm">
                Tạo Ticket
            </button>
        </div>
    </form>
