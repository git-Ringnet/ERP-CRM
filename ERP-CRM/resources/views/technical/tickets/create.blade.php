...
@section('content')
    ...
    <div class="" x-data="{ 
        ...
        <a href="{{ route('technical-tickets.index') }}"
            class="text-gray-500 hover:text-gray-700 transition-colors flex items-center space-x-2">
            <i class="fas fa-arrow-left text-lg"></i>
            <h2 class="text-lg font-bold text-gray-900">Trở về</h2>
        </a>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Priority -->
                        <div>
                            <label for="priority" class="block text-sm font-semibold text-gray-700 mb-1">Priority (SLA phản
                                hồi) <span class="text-red-500">*</span></label>
                            <select name="priority" id="priority" required
                                ...
                                <option value="medium" {{ old('priority', 'medium') === 'medium' ? 'selected' : '' }}>Medium
                                    (Phản hồi tối đa 8 giờ làm việc)</option>
                                <option value="high" {{ old('priority') === 'high' ? 'selected' : '' }}>High (Phản hồi tối đa
                                    4 giờ làm việc)</option>
                            </select>
                            @error('priority')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="sla_deadline" class="block text-sm font-semibold text-gray-700 mb-1">Due Date (Thời
                                gian yêu cầu xử lý)</label>
                            <input type="datetime-local" name="sla_deadline" id="sla_deadline"
                                class="w-full border-gray-200 rounded-lg text-sm focus:border-primary focus:ring-primary"
                                value="{{ old('sla_deadline') }}">
                            <span class="text-xs text-gray-400 mt-1 block">Để trống để hệ thống tự động tính toán SLA (4h
                                hành chính cho High, 8h hành chính cho Medium).</span>
                        </div>
                    </div>
