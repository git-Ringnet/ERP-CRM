                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    @foreach($request->suppliers as $supplier)
                                        <span
                                            class="inline-block bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded mr-1 mb-1">{{ $supplier->name }}</span>
                                    @endforeach
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">{{ $request->quotations->count() }}</span>
                                </td>
                                <td class="px-4 py-3">
                                    ...
                                    @elseif($request->status == 'received')
                                        <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">Đã nhận báo
                                            giá</span>
                                    @elseif($request->status == 'converted')
                                        <span class="px-2 py-1 text-xs rounded-full bg-purple-100 text-purple-800">Đã chuyển
                                            PO</span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center space-x-1">
                                        <a href="{{ route('purchase-requests.show', $request) }}"
                                            class="inline-flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200"
                                            title="Xem">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($request->status == 'draft')
                                            <a href="{{ route('purchase-requests.edit', $request) }}"
                                                class="inline-flex items-center justify-center w-8 h-8 bg-yellow-100 text-yellow-600 rounded-lg hover:bg-yellow-200"
                                                title="Sửa">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('purchase-requests.send', $request) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                <button type="submit"
                                                    class="inline-flex items-center justify-center w-8 h-8 bg-green-100 text-green-600 rounded-lg hover:bg-green-200"
                                                    title="Gửi NCC" onclick="return confirm('Gửi yêu cầu báo giá cho các NCC?')">
                                        ...
                                        @if(in_array($request->status, ['draft', 'cancelled']))
                                            <form action="{{ route('purchase-requests.destroy', $request) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center justify-center w-8 h-8 bg-red-100 text-red-600 rounded-lg hover:bg-red-200"
                                                    title="Xóa">
                                        ...
                                        @if($request->quotations->count() > 0)
                                            <a href="{{ route('supplier-quotations.index', ['purchase_request_id' => $request->id]) }}"
                                                class="inline-flex items-center justify-center w-8 h-8 bg-purple-100 text-purple-600 rounded-lg hover:bg-purple-200"
                                                title="Xem báo giá">
