                ...
                @if($purchaseRequest->status == 'draft')
                    ...
                    <form action="{{ route('purchase-requests.send', $purchaseRequest) }}" method="POST" class="inline">
                        ...
                        <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700"
                            onclick="return confirm('Gửi yêu cầu báo giá cho các NCC?')">
                            <i class="fas fa-paper-plane mr-2"></i> Gửi NCC
                        </button>
                    </form>
                @endif
                @if(in_array($purchaseRequest->status, ['sent', 'received']))
                    <a href="{{ route('supplier-quotations.create', ['purchase_request_id' => $purchaseRequest->id]) }}"
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
                    ...
                    @if($purchaseRequest->quotations->count() >= 2)
                        <a href="{{ route('supplier-quotations.compare', ['ids' => $purchaseRequest->quotations->pluck('id')->toArray()]) }}"
