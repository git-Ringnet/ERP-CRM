...
@section('title', 'Import bảng giá NCC')
    ...
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <div id="step1" class="step-content p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="supplier_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-transparent">
                        <option value="">Chọn nhà cung cấp...</option>
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" data-type="{{ strtolower($supplier->name) }}">{{ $supplier->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                ...
                <div>
                    ...
                    <input type="number" id="exchange_rate" value="25000" min="1" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tên bảng giá</label>
                    <input type="text" id="price_list_name" placeholder="VD: Fortinet APAC 2025" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <div id="dropzone" class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-primary transition-colors cursor-pointer">
                    ...
                    <button type="button" onclick="document.getElementById('excel_file').click()" class="mt-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark">
            ...
            <div class="mt-6 flex justify-end">
                <button type="button" id="btn_to_step2" disabled class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div id="step2" class="step-content p-6 hidden">
            ...
            <div class="mt-6 flex justify-between">
                ...
                <button type="button" id="btn_to_step3" disabled class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:opacity-50">
                    Tiếp theo <i class="fas fa-arrow-right ml-2"></i>
                </button>
            </div>
        </div>
        ...
        <div id="step3" class="step-content p-6 hidden">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="autoDetectAllSheets()" class="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm">
                        <i class="fas fa-magic mr-1"></i>Tự động tất cả sheet
                    </button>
                    <button type="button" onclick="autoDetectMapping()" class="px-3 py-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 text-sm">
                        <i class="fas fa-magic mr-1"></i>Tự động nhận diện
                    </button>
                    <button type="button" onclick="applyMappingToAll()" class="px-3 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 text-sm">
            ...
            <div id="quick_apply_info" class="hidden bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                <p class="text-green-700 text-sm">
                    ...
                    Đã áp dụng mapping tự động cho <strong id="auto_mapped_count">0</strong> sheet. Bạn có thể chỉnh sửa từng sheet nếu cần.
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" id="header_row" value="1" min="1" max="20" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bắt đầu dữ liệu từ dòng</label>
                    <input type="number" id="data_start_row" value="2" min="2" class="w-full border border-gray-300 rounded-lg px-3 py-2" readonly>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">SKU / Mã SP <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            ...
                            <select id="map_product_name" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Mô tả</label>
                            <select id="map_description" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Giá (Price) <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            ...
                            <select id="map_category" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="bg-orange-50 rounded-lg p-4">
                    <h4 class="font-medium text-gray-900 mb-3"><i class="fas fa-file-contract text-orange-600 mr-2"></i>Giá theo thời hạn hợp đồng</h4>
                    <div class="space-y-3">
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Giá 1 năm</label>
                            <select id="map_price_1yr" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Giá 2 năm</label>
                            <select id="map_price_2yr" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Giá 3 năm</label>
                            <select id="map_price_3yr" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Giá 4 năm</label>
                            <select id="map_price_4yr" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Giá 5 năm</label>
                            <select id="map_price_5yr" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Chọn cột --</option>
                            </select>
                        </div>
                    </div>
            ...
            <div class="mt-6 flex justify-between">
                ...
                <button type="button" id="btn_to_step4" class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark">
                    Tiếp theo <i class="fas fa-arrow-right ml-2"></i>
                </button>
            </div>
        </div>
        ...
        <div id="step4" class="step-content p-6 hidden">
            <div class="bg-gray-50 rounded-lg p-6">
                <h4 class="font-medium text-gray-900 mb-4"><i class="fas fa-clipboard-check mr-2"></i>Tổng kết Import</h4>
                    ...
                    <div class="space-y-2">
                        ...
                        <label class="flex items-start gap-3 p-2 rounded hover:bg-gray-50 cursor-pointer">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Cập nhật giá nếu SKU đã tồn tại, thêm mới nếu chưa có</p>
                            </div>
                        </label>
                        <label class="flex items-start gap-3 p-2 rounded hover:bg-gray-50 cursor-pointer">
            ...
            <div class="mt-6 flex justify-between">
                <button type="button" onclick="goToStep(3)" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
                    <i class="fas fa-arrow-left mr-2"></i>Quay lại
                </button>
                <button type="button" id="btn_import" onclick="executeImport()" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
        ...
        <div id="loading_overlay" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-8 text-center">
                ...
                <div class="w-64 h-2 bg-gray-200 rounded-full mt-4">
                    <div id="progress_bar" class="h-full bg-primary rounded-full transition-all" style="width: 0%"></div>
                </div>
            </div>
        </div>
    </div>
...
<style>
.step-item { display: flex; align-items: center; gap: 8px; padding: 8px 16px; background: #f3f4f6; border-radius: 20px; color: #6b7280; font-size: 14px; }
.step-item.active { background: #3b82f6; color: white; }
.step-item.completed { background: #10b981; color: white; }
.step-number { width: 24px; height: 24px; border-radius: 50%; background: rgba(255,255,255,0.3); display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 12px; }
.step-line { width: 40px; height: 2px; background: #d1d5db; }
