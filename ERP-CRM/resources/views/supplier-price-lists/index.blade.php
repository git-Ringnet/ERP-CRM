...
@section('title', 'Bảng giá nhà cung cấp')
@section('page-title', 'Bảng giá nhà cung cấp')
...
<div class="bg-white rounded-lg shadow-sm">
    ...
    <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            <!-- Filter by Supplier -->
            <select name="supplier_id" onchange="window.location.href='{{ route('supplier-price-lists.index') }}?supplier_id='+this.value+'&search={{ request('search') }}'" 
    ...
    <div class="mx-4 mt-4 bg-blue-50 border-l-4 border-blue-500 p-3 rounded">
        <p class="text-blue-700 text-sm">
            ...
            Import bảng giá từ file Excel của nhà cung cấp (Fortinet, Cisco, HP...). Hỗ trợ file có nhiều sheet với cấu trúc khác nhau.
        </p>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 whitespace-nowrap">
                        <a href="{{ route('supplier-price-lists.show', $priceList) }}" class="font-medium text-blue-600 hover:text-blue-800">
                            {{ $priceList->code }}
                        </a>
                    </td>
                    <td class="px-4 py-3">
                        ...
                        @if($priceList->file_name)
                            <div class="text-xs text-gray-500"><i class="fas fa-file-excel text-green-600"></i> {{ $priceList->file_name }}</div>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            <form action="{{ route('supplier-price-lists.toggle', $priceList) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="p-2 {{ $priceList->is_active ? 'text-gray-600 hover:bg-gray-50' : 'text-green-600 hover:bg-green-50' }} rounded" 
                                        title="{{ $priceList->is_active ? 'Tắt' : 'Bật' }}">
                                    <i class="fas {{ $priceList->is_active ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
                                </button>
                            </form>
                            <form action="{{ route('supplier-price-lists.destroy', $priceList) }}" method="POST" class="inline"
                ...
                <tr>
                    <td colspan="9" class="px-4 py-8 text-center text-gray-500">
                        ...
                        <a href="{{ route('supplier-price-lists.import') }}" class="text-blue-600 hover:underline mt-2 inline-block">
