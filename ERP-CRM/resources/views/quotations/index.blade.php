...
<div class="bg-white rounded-lg shadow-sm">
    ...
    <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            <div class="flex items-center gap-2">
                <select name="status" onchange="window.location.href='{{ route('quotations.index') }}?status='+this.value+'&search={{ request('search') }}'" 
                    ...
                    <option value="accepted" {{ request('status') == 'accepted' ? 'selected' : '' }}>Khách chấp nhận</option>
                    <option value="declined" {{ request('status') == 'declined' ? 'selected' : '' }}>Khách từ chối</option>
                    <option value="converted" {{ request('status') == 'converted' ? 'selected' : '' }}>Đã chuyển ĐH</option>
                </select>
            </div>
        </div>
    ...
    <div class="mx-4 mt-4 bg-blue-50 border-l-4 border-blue-500 p-3 rounded">
        <p class="text-blue-700 text-sm"><i class="fas fa-info-circle mr-2"></i>Quy trình: Nháp → Chờ duyệt → Đã duyệt → Gửi khách → Chấp nhận/Từ chối → Chuyển đơn hàng</p>
    </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Hạn BG</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">
                        <a href="{{ route('quotations.show', $quotation) }}" class="font-medium text-blue-600 hover:text-blue-800">
                            {{ $quotation->code }}
                        </a>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-sm {{ $quotation->isExpired() ? 'text-red-600 font-medium' : 'text-gray-500' }}">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <a href="{{ route('quotations.print', $quotation) }}" target="_blank"
                               class="p-2 text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 hover:text-gray-700 transition-colors" title="In">
    ...
    <div class="md:hidden divide-y divide-gray-200">
        ...
        <div class="p-4 hover:bg-gray-50">
            <div class="flex justify-between items-start mb-2">
                <div class="flex-1">
                    <a href="{{ route('quotations.show', $quotation) }}" class="font-medium text-blue-600 hover:text-blue-800">{{ $quotation->code }}</a>
            ...
            <div class="space-y-1 text-sm text-gray-600 mb-3">
                <div><i class="fas fa-calendar w-4"></i> {{ $quotation->date->format('d/m/Y') }} - {{ $quotation->valid_until->format('d/m/Y') }}</div>
