        ...
        <div class="p-4 border-b border-gray-200 bg-gray-50">
            <form action="{{ route('transfers.index') }}" method="GET">
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-x-4 gap-y-3 items-end">
                    ...
                    <!-- Warehouse -->
                    <div class="flex flex-col">
                        <label class="block text-xs font-medium text-gray-500 mb-1">Kho</label>
                    ...
                    <div class="flex gap-2 h-[38px]">
                        ...
                        <a href="{{ route('transfers.index') }}" class="px-3 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-sm font-medium">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    @if($transfer->status === 'pending')
                                        ...
                                        <button
                                            onclick="confirmApprove('{{ route('transfers.approve', $transfer) }}', 'phiếu chuyển kho')"
                                            class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200" title="Duyệt">
                                            <i class="fas fa-check"></i>
                                        </button>
                                        <button
                                            onclick="confirmReject('{{ route('transfers.reject', $transfer) }}', 'phiếu chuyển kho')"
                                    ...
                                    @elseif($transfer->status === 'rejected')
                                        <form action="{{ route('transfers.destroy', $transfer) }}" method="POST" class="inline-block"
                                            onsubmit="return confirm('Bạn có chắc chắn muốn xóa phiếu chuyển kho này?')">
                                            @csrf
                                            @method('DELETE')
