...
@section('title', 'Sửa Danh mục Phụ cấp/Khấu trừ')
...
<div class="">
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Sửa danh mục: {{ $salaryComponent->name }}</h2>
            <a href="{{ route('salary-components.index') }}" class="text-gray-500 hover:text-gray-700 flex items-center">
                <i class="fas fa-arrow-left mr-2"></i>Quay lại
            </a>
        </div>

        <form action="{{ route('salary-components.update', $salaryComponent->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                            <option value="allowance" {{ old('type', $salaryComponent->type) == 'allowance' ? 'selected' : '' }}>Phụ cấp (Cộng vào lương)</option>
                            <option value="deduction" {{ old('type', $salaryComponent->type) == 'deduction' ? 'selected' : '' }}>Khấu trừ (Trừ vào lương)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="amount_type" id="amount_type" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                            <option value="fixed" {{ old('amount_type', $salaryComponent->amount_type) == 'fixed' ? 'selected' : '' }}>Số tiền cố định (VNĐ)</option>
                            <option value="percentage" {{ old('amount_type', $salaryComponent->amount_type) == 'percentage' ? 'selected' : '' }}>Phần trăm (%) lương cơ bản</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="any" min="0" name="default_amount" id="default_amount" value="{{ old('default_amount', $salaryComponent->default_amount) }}" required
                    ...
                    <p class="mt-1 text-xs text-gray-500">Mức tiền / Phần trăm mặc định áp dụng chung. Thay đổi sẽ KHÔNG tự động cập nhật mức đã gán trước đó cho nhân viên.</p>
                    @error('default_amount')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <!-- Chịu thuế -->
                <div class="flex items-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <input type="checkbox" name="is_taxable" id="is_taxable" value="1" {{ old('is_taxable', $salaryComponent->is_taxable) ? 'checked' : '' }}
