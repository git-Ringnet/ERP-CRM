                ...
                <div id="productList" class="space-y-3">
                    ...
                    <div class="product-item bg-gray-50 p-3 rounded-lg">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Thành tiền (<span class="currency-symbol">{{ $sale->currency ? ($sale->currency->symbol ?? $sale->currency->code) : '₫' }}</span>)</label>
                                ...
                                <div class="md:col-span-3">
                                    @php
                                        ...
                                        if ($sale->currency && !$sale->currency->is_base && $sale->exchange_rate) {
                                            $expenseAmount = $expenseAmount / $sale->exchange_rate;
                                        }
                                        $amountDecimals = $sale->currency && !$sale->currency->is_base ? ($sale->currency->decimal_places ?? 2) : 0;
                                    @endphp
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Số tiền (<span class="currency-symbol">{{ $sale->currency ? ($sale->currency->symbol ?? $sale->currency->code) : '₫' }}</span>)</label>
                                    <input type="text" name="expenses[{{ $index }}][amount]" value="{{ number_format($expenseAmount, $amountDecimals, '.', ',') }}"
        ...
        if (!container.dataset.initialized) {
            initSearchableSelect(container, (opt) => {
                ...
                if (priceInput && opt.dataset.price) {
                    ...
                    calculateRowTotal(); // Recalculate all rows
                }
