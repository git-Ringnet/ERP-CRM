...
@section('page-title', 'Chỉnh sửa Phiếu Xuất Kho')

@section('content')
<div class="bg-white rounded-lg shadow-sm">
    <div class="p-4 border-b border-gray-200 flex justify-between items-center">
        <h2 class="text-lg font-semibold text-gray-900">Chỉnh sửa phiếu xuất: {{ $export->code }}</h2>
        <a href="{{ route('exports.show', $export) }}" class="text-gray-600 hover:text-gray-900">
    ...
    @else
    <form action="{{ route('exports.update', $export) }}" method="POST" class="p-4" onsubmit="return validateForm()">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div>
                ...
                <input type="date" name="date" value="{{ old('date', $export->date->format('Y-m-d')) }}" required
            ...
            <div>
                ...
                <select name="employee_id" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg">
                    ...
                    @foreach($employees as $employee)
                        <option value="{{ $employee->id }}" {{ $export->employee_id == $employee->id ? 'selected' : '' }}>
                            {{ $employee->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="border-t border-gray-200 pt-4">
            <div class="flex justify-between items-center mb-3">
                ...
                <button type="button" onclick="addItem()" class="px-4 py-2 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600">
        ...
        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
            ...
            <button type="submit" class="px-4 py-2 text-sm text-white bg-orange-500 rounded-lg hover:bg-orange-600">
                <i class="fas fa-save mr-1"></i> Cập nhật phiếu xuất
            </button>
        </div>
    </form>
    @endif
...
const defaultWarehouseId = {{ $export->warehouse_id }};
let stockCache = {};
...
function addItem(existingData = null) {
    ...
    itemDiv.innerHTML = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-3">
            ...
            <div class="md:col-span-3">
                ...
                <input type="text" value="${formatNumber(existingData ? existingData.unit_price || 0 : 0)}" 
                       class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded unit-price-display" 
                       oninput="onPriceInput(this, ${itemIndex})">
                <input type="hidden" name="items[${itemIndex}][unit_price]" value="${existingData ? existingData.unit_price || 0 : 0}" class="unit-price-raw">
            </div>
            <div class="md:col-span-3">
                <label class="block text-xs font-medium text-gray-600 mb-1">Thành tiền</label>
                <input type="text" value="${formatNumber(existingData ? existingData.total || 0 : 0)}" 
                       readonly class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded bg-gray-100 total-display">
                <input type="hidden" name="items[${itemIndex}][total]" value="${existingData ? existingData.total || 0 : 0}" class="total-raw">
            </div>
        ...
        <div id="stockInfo_${itemIndex}" class="mb-3 hidden">
            <div class="flex items-center justify-between p-2 bg-white rounded border border-gray-200">
                <span class="text-xs font-medium text-gray-600"><i class="fas fa-warehouse mr-1"></i>Tồn kho:</span>
    ...
    setTimeout(() => {
        const lastItem = document.querySelector(`[data-index="${currentIdx}"]`);
        if (lastItem) {
...
async function loadStockInfo(itemIdx) {
    ...
    const avgCost = data.avg_cost || 0; // Assuming avg_cost is returned
...
function validateQuantity(itemIdx) {
    ...
    if (qty > totalStock) {
        ...
        warningEl.innerHTML = `<i class="fas fa-exclamation-triangle mr-1"></i>Cần chọn thêm ${needSerials} serial`;
...
function validateForm() {
    ...
    items.forEach((item, idx) => {
        ...
        if (!productId) {
            hasError = true;
            errorMessages.push(`Sản phẩm #${idx + 1}: Chưa chọn sản phẩm.`);
            return;
        }
